...
public class CompoundShortDataset extends AbstractCompoundDataset {
	 ...
	 * @param itemSize
	 */
	CompoundShortDataset(final int itemSize) {
		isize = itemSize;
	}
	 ...
	 * @param shape
	 */
	CompoundShortDataset(final int itemSize, final int[] shape) {
		isize = itemSize;
		if (shape != null) {
	 ...
	 * @param dataset
	 */
	CompoundShortDataset(final CompoundShortDataset dataset) {
		isize = dataset.isize;
	 ...
	 * @param dataset
	 */
	CompoundShortDataset(final CompoundDataset dataset) {
	 ...
	 */
	CompoundShortDataset(final int itemSize, final short[] data, int... shape) { // PRIM_TYPE
		isize = itemSize;
		if (data != null) {
	 ...
	 * @param datasets
	 */
	CompoundShortDataset(final Dataset... datasets) {
	 ...
	 * @param dataset
	 */
	CompoundShortDataset(final int itemSize, final boolean repeat, final Dataset dataset) {
		isize = itemSize;
	 ...
	 */
	static CompoundShortDataset createFromObject(final Object obj) {
	...
	public static CompoundShortDataset createFromObject(final int itemSize, final Object obj) {
		...
		CompoundShortDataset ds = new CompoundShortDataset(itemSize, result.getData(), null);
		if (zeroRank) {
	 ...
	 */
	static CompoundShortDataset createRange(final int itemSize, final double stop) {
	 ...
	 * @return a new 1D dataset, filled with values determined by parameters
	 */
	static CompoundShortDataset createRange(final int itemSize, final double start, final double stop,
			final double step) {
	 ...
	 */
	static CompoundShortDataset ones(final int itemSize, final int... shape) {
	...
	public static CompoundShortDataset createCompoundDatasetWithLastDimension(final Dataset a, final boolean shareData) {
		...
		if (shape == null) {
			return new CompoundShortDataset(0);
		}
	...
	public Object getObject() {
		return getShortArray(); // PRIM_TYPE
	}
	...
	public void set(final Object obj) {
		setItem(DTypeUtils.toShortArray(obj, isize)); // CLASS_TYPE
	}
	 ...
	 * @since 2.0
	 */
	public void setItem(final short[] d) { // PRIM_TYPE
		if (d.length > isize) {
	...
	CompoundShortDataset setSlicedView(Dataset view, Dataset d) {
		...
		if (is > 1) {
			if (d.getElementsPerItem() == 1) {
				while (it.hasNext()) {
					final short bv = (short) it.bLong; // PRIM_TYPE // BCAST_WITH_CAST d.getElementLongAbs(it.bIndex);
					data[it.aIndex] = bv;
					for (int j = 1; j < is; j++) {
						data[it.aIndex + j] = bv;
					}
				...
				while (it.hasNext()) {
					data[it.aIndex] = (short) it.bLong; // BCAST_WITH_CAST d.getElementLongAbs(it.bIndex);
					for (int j = 1; j < is; j++) {
