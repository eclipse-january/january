...
/**
 * This class tests the EMFTreeComposite. It checks that the 
 * class can be constructed from a valid EClass instance, that 
 * the tree can be modified through the addition of children, and 
 * that the tree can be copied and cloned. 
 * 
...
public class EMFTreeCompositeTester {
	...
	/**
	 * Reference to a list of EMFTreeComposites 
	 * that will be created from the Ship Order schema
	 */
	...
	/**
	 * This method is run before all tests and initializes a set 
	 * of EMFTreeComposites for testing.
	 */
	@Before
	public void before() {
		// Local Declarations
		...
		EPackage shipOrderPackage = (EPackage) processor.getEPackageRegistry().values()
				.toArray()[0];
	...
	/**
	 * Check that we can correctly construct EMFTreeComposites 
	 * from EClass objects.
	 */
	@Test
	...
	public void checkAddRemoveChildren() {
		...
		assertTrue(((DataComponent) clonedOrder.getActiveDataNode()).retrieveEntry(
				"orderperson").setValue("McCaskey"));
		...
		assertFalse("hello".equals(((DataComponent) anotherClonedItem.getActiveDataNode())
		...
		EMFTreeComposite anotherClonedShipTo = (EMFTreeComposite) shipTo.clone();
		assertEquals(3, clonedOrder.getNumberOfChildren());
	...
	public void checkCopying() {
		...
		assertTrue("hello".equals(((DataComponent) shipOrder.getActiveDataNode())
				.retrieveEntry("orderperson").getValue()));
		assertFalse("hello".equals(((DataComponent) clonedOrder.getActiveDataNode())
				.retrieveEntry("orderperson").getValue()));
		
		assertTrue(((DataComponent) clonedOrder.getActiveDataNode())
				.retrieveEntry("orderperson").setValue("helloworld"));
		assertTrue("helloworld".equals(((DataComponent) clonedOrder.getActiveDataNode())
				.retrieveEntry("orderperson").getValue()));
		assertFalse("helloworld".equals(((DataComponent) shipOrder.getActiveDataNode())
				.retrieveEntry("orderperson").getValue()));
