	...
	public CompoundShortDataset iadd(final Object b) {
		...
		if (bds.getSize() == 1) {
			...
			if (is == 1) {
				...
				while (it.hasNext()) {
					for (int i = 0; i < isize; i++) {
						data[it.index + i] += (short) bds.getElementLongAbs(i); // GET_ELEMENT_WITH_CAST
					}
				}
			} else {
			...
			if (is == 1) {
				while (it.hasNext()) {
					...
					data[it.aIndex] += (short) db; // ADD_CAST
					for (int i = 1; i < isize; i++) {
						data[it.aIndex + i] += (short) db; // ADD_CAST
					}
				}
			} else if (is == isize) {
				while (it.hasNext()) {
	...
	public CompoundShortDataset isubtract(final Object b) {
		...
		if (bds.getSize() == 1) {
			...
			if (is == 1) {
				...
				while (it.hasNext()) {
					for (int i = 0; i < isize; i++) {
						data[it.index + i] = (short) (data[it.index + i] - db); // INT_USE // ADD_CAST
					}
				}
			} else if (is == isize) {
				while (it.hasNext()) {
					for (int i = 0; i < isize; i++) {
						data[it.index + i] = (short) (data[it.index + i] - bds.getElementDoubleAbs(i)); // INT_USE // ADD_CAST
					}
				}
			} else {
				throw new IllegalArgumentException("Argument does not have same number of elements per item or is not a non-compound dataset");
			...
			if (is == 1) {
				while (it.hasNext()) {
					final double db = it.bDouble;
					for (int i = 0; i < isize; i++) {
						data[it.aIndex + i] = (short) (it.aDouble - db); // INT_USE // ADD_CAST
					}
				}
			} else if (is == isize) {
				while (it.hasNext()) {
