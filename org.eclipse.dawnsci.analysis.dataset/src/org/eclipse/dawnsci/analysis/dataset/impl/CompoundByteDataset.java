...
public class CompoundByteDataset extends AbstractCompoundDataset {
	...
	public CompoundByteDataset iadd(final Object b) {
		...
		if (bds.getSize() == 1) {
			...
			if (is == 1) {
				final byte db = (byte) bds.getElementLongAbs(0); // PRIM_TYPE // GET_ELEMENT_WITH_CAST
				while (it.hasNext()) {
					for (int i = 0; i < isize; i++) {
						data[it.index + i] += db;
					}
				...
				while (it.hasNext()) {
					for (int i = 0; i < isize; i++) {
						data[it.index + i] += (byte) bds.getElementLongAbs(i); // GET_ELEMENT_WITH_CAST
					}
				}
			} else {
			...
			if (is == 1) {
				while (it.hasNext()) {
					...
					data[it.aIndex] += (byte) db; // ADD_CAST
					for (int i = 1; i < isize; i++) {
						data[it.aIndex + i] += (byte) db; // ADD_CAST
					}
				}
			} else if (is == isize) {
				while (it.hasNext()) {
	...
	public CompoundByteDataset isubtract(final Object b) {
		...
		if (bds.getSize() == 1) {
			...
			if (is == 1) {
				...
				while (it.hasNext()) {
					for (int i = 0; i < isize; i++) {
						data[it.index + i] = (byte) (data[it.index + i] - db); // INT_USE // ADD_CAST
					}
				}
			} else if (is == isize) {
				while (it.hasNext()) {
					for (int i = 0; i < isize; i++) {
						data[it.index + i] = (byte) (data[it.index + i] - bds.getElementDoubleAbs(i)); // INT_USE // ADD_CAST
					}
				}
			} else {
				throw new IllegalArgumentException("Argument does not have same number of elements per item or is not a non-compound dataset");
			...
			if (is == 1) {
				while (it.hasNext()) {
					final double db = it.bDouble;
					for (int i = 0; i < isize; i++) {
						data[it.aIndex + i] = (byte) (it.aDouble - db); // INT_USE // ADD_CAST
					}
				}
			} else if (is == isize) {
				while (it.hasNext()) {
